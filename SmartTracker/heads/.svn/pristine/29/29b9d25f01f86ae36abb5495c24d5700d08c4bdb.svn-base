...
public class SensorComController extends ModuleController implements ObservationProvider, ChestBeltListener {
	...
	public void batteryStatus(int value, int timestamp) {
		log.info("batteryStatus: " +String.valueOf(value));
		if(tracking){
			final TrackerEvent te = new TrackerEvent();
			te.setTimestamp(System.currentTimeMillis());
			te.setProperty("battery.level", String.valueOf(value));
	...
	public void heartRate(int value, int timestamp) {
		log.info("Heart Rate: " +String.valueOf(value/10));
		if(tracking){
			final TrackerEvent te = new TrackerEvent();
			te.setTimestamp(System.currentTimeMillis());
			te.setProperty("heartrate", String.valueOf(value/10));
			te.setType(TrackerEvent.EVENT_CUSTOM);
			te.setSendStatus(TrackerEvent.SEND_TODO);
			Runnable r = new Runnable() {
				@Override
	...
	@Override
	public void indication(int value, int arg1) {
		log.info("Indication: " +String.valueOf(value));
		if(tracking){
			final TrackerEvent te = new TrackerEvent();
			te.setTimestamp(System.currentTimeMillis());
			if (value >= 1 && value <= 6) { 
				te.setProperty("sensor.position", String.valueOf(value));
			} else if (value >= 10 && value <= 13) {
				te.setProperty("sensor.activity", String.valueOf(value-10));
			}
			te.setType(TrackerEvent.EVENT_CUSTOM);
			te.setSendStatus(TrackerEvent.SEND_TODO);
			Runnable r = new Runnable() {
	...
	@Override
	public void skinTemperature(int value, int arg1) {
		log.info("Skin Temperature: " +String.valueOf(value/10));
		if(tracking){
			final TrackerEvent te = new TrackerEvent();
			te.setTimestamp(System.currentTimeMillis());
			te.setProperty("temperature", String.valueOf(value/10));
			te.setType(TrackerEvent.EVENT_CUSTOM);
			te.setSendStatus(TrackerEvent.SEND_TODO);
			Runnable r = new Runnable() {
				@Override
