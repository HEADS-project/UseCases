...
import no.tellu.tracker.heads.HeadsActivity;
...
public class BluetoothManagementService extends Service implements ConnectionTaskReceiver, ChestBeltCallback {
	...
	private void endConnection(String address, boolean intentional) {
		...
		Toast.makeText(getApplicationContext(), "Disconnected from " + d.getName(), Toast.LENGTH_LONG).show();
		...
		connectionNotification.setLatestEventInfo(getApplicationContext(), getString(R.string.app_name), "Disconnected", pNotificationIntent);
	...
	public void onConnectionSuccess(String name, String address) {
		...
		if (newSession != null) {
			...
			TelluApp app = (TelluApp)getApplicationContext();
			
			SensorComController sc = (SensorComController)app.getModuleController(SensorComController.MODULE_ID);
			if(sc == null){
				Intent in = new Intent(getApplicationContext(), HeadsActivity.class);
				in.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
				startActivity(in);
			}
			sc = (SensorComController)app.getModuleController(SensorComController.MODULE_ID);
			
			ChestBeltGraphBufferizer bufferizer = new ChestBeltGraphBufferizer(this, address);
			graphBufferizers.put(address, bufferizer);
			newSession.addChestBeltListener(sc);
