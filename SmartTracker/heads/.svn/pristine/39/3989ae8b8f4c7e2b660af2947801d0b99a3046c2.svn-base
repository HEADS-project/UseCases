...
public class DevicesListActivity extends ListActivity {
	...
	protected void onListItemClick(ListView l, View v, int position, long id) {
		...
		if (v.getId() == R.id.list_footer) {
			...
			if (d.isConnected()) {
				...
				TelluApp app = (TelluApp)getApplicationContext();
				HeadsControl hc = (HeadsControl)app.getModuleController(HeadsControl.MODULE_ID);
				hc.registerDevice(d.getName(), d.getAddress()); //TODO sjekk denne
				
				Intent i = new Intent(getApplicationContext(), HeadsActivity.class);
				startActivity(i);
	...
	private void connect(Device device) {
		if (!BluetoothAdapter.getDefaultAdapter().isEnabled()) {
			Toast.makeText(getApplicationContext(), "Error: Bluetooth is disabled", Toast.LENGTH_LONG).show();
			return;
		}
		...
		switch (requestCode) {
			case REQUEST_ENABLE_BT:
				if (resultCode == RESULT_OK) { 
					Log.i(TAG, "Bluetooth succesfully activated.");
					getBondedDevice();
	...
	private void deviceConnected(Device device) {
		...
		if (changeActivity) {
			...
			HeadsControl hc = (HeadsControl)app.getModuleController(HeadsControl.MODULE_ID);
			hc.registerDevice(device.getName(), device.getAddress());
